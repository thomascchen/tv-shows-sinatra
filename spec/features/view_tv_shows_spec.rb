...
feature "user views list of TV shows" do
  ...
  scenario "view details for a TV show" do
    married_with_children = TelevisionShow.create!({
        title: "Married... with Children", network: "Fox",
        starting_year: 1987, ending_year: 1997,
        genre: "Comedy", synopsis: "A Show about people"
      })

    id = married_with_children.id

    visit "/television_shows/#{id}"

    expect(page).to have_content(married_with_children.title)
    expect(page).to have_content(married_with_children.network)
    expect(page).to have_content(married_with_children.genre)
    expect(page).to have_content(married_with_children.starting_year)
    expect(page).to have_content(married_with_children.ending_year)
    expect(page).to have_content(married_with_children.synopsis)
  end
